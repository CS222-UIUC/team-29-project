...
describe('Chat Component with JWT Authentication', () => {
  ...
  beforeEach(() => {
    ...
    (apiClient as jest.Mock).mockImplementation((url, options) => {
      if (url === '/api/models') {
  ...
  test('sends chat message with JWT authentication', async () => {
    ...
    await waitFor(() => {
      expect(apiClient).toHaveBeenCalledWith('/api/chat', expect.objectContaining({
        method: 'POST',
  ...
  test('gets conversation details without sending user_id query param', async () => {
    ...
    await waitFor(() => {
      const conversationDetailCalls = (apiClient as jest.Mock).mock.calls.filter(
        call => call[0].startsWith('/api/conversations/conv1')
      );
  ...
  test('handles unauthorized errors from API', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Authentication error fetching conversations/i)).toBeInTheDocument();
    });
  });
});