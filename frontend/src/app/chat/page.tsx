...
export default function Chat() {
  ...
  const [conversationMetadataList, setConversationMetadataList] = useState<
    ConversationMetadata[] | null
  >(null);
  const [activeConversationContent, setActiveConversationContent] =
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401 || response.status === 403) {
          ...
          throw new Error(
            `Failed to fetch conversation list: ${response.status}`,
          );
        }
      ...
      setError(
        `Failed to load conversation list: ${err instanceof Error ? err.message : String(err)}`,
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          activeConversationContent?.messages.at(-1)?.id === tempUserMessageId
        ) {
          setActiveConversationContent((prev) =>
      ...
      setConversationMetadataList((prevList) => {
        ...
        const existingIndex = prevList.findIndex(
          (meta) => meta.id === realConversationId,
        );
        ...
        if (existingIndex !== -1) {
          ...
          const newMeta: ConversationMetadata = {
            ...
            user_id: userId,
            title:
              activeConversationContent?.title ||
              messageToSend.substring(0, 30) + "...", // Use title from active content or generate
            created_at: now,
            updated_at: now,
  ...
  const handleBranch = async (messageId: string) => {
    ...
    try {
      const response = await apiClient(
        `/api/conversations/${activeConversationId}/branch`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ detail: "Branching failed" }));
        throw new Error(
          `Branching error ${response.status}: ${errorData.detail || response.statusText}`,
        );
      }
      ...
      setConversationMetadataList((prevList) => {
        const newMeta: ConversationMetadata = {
          ...
          branch_point_message_id:
            newBranchConversation.branch_point_message_id,
        };
  ...
  const getBranchChildren = (messageId: string): ConversationMetadata[] => {
    ...
    return conversationMetadataList.filter(
      (meta) =>
  ...
  return (
    ...
    <div className="flex h-screen font-[family-name:var(--font-geist-sans)]">
      ...
      {status === "loading" && (
        <div className="w-64 pr-4 border-r border-gray-300 dark:border-gray-700 flex items-center justify-center">
          <p>Loading session...</p>
        </div>
      )}
      {status === "unauthenticated" && (
        <div className="w-64 pr-4 border-r border-gray-300 dark:border-gray-700 flex flex-col p-4 items-center justify-center text-center">
          <p className="text-sm mb-4">
            Sign in to view and save conversations.
          </p>
      ...
      <div className="flex-1 flex flex-col p-4 md:p-8 overflow-y-auto">
        ...
        <main className="flex-1 flex flex-col max-w-3xl mx-auto w-full">
          <h1
            className="text-xl md:text-3xl font-bold mb-4 truncate"
            title={activeConversationContent?.title || "Chat"}
          >
            {activeConversationContent?.title ||
              (activeConversationId ? "Loading..." : "Select or Start a Chat")}
          </h1>
          ...
          <div className="bg-black/[.05] dark:bg-white/[.06] rounded-lg p-4 md:p-6 mb-6 flex-1 flex flex-col space-y-4 overflow-y-auto min-h-[200px]">
            ...
            {!isLoadingContent && status !== "authenticated" && (
              <p className="text-gray-500 text-center">
                  ...
                  <div className="absolute top-0 -right-2 transform translate-x-full flex flex-col items-center opacity-50 hover:opacity-100 transition-opacity">
                    ...
                    {hasBranches && (
                      <span
                        ...
                        title={`Branched ${branchChildren.length} time(s)`}
                      >
                ...
                {isLoadingModels ? (
                  ...
                  Object.keys(models).map((provider) => (
                    ...
                    >
                      ...
                      {!isProviderAvailable(models, provider) &&
                        "(API key required)"}
                    </option>
                  ))
                )}
            ...
            <div className="flex-1">
              ...
              <select
                ...
                disabled={
                  ...
                  !models ||
                  !selectedProvider ||
                  !models[selectedProvider]?.models.length
                }
              >
                {isLoadingModels ? (
                  <option>Loading...</option>
                ) : !models || !models[selectedProvider]?.models.length ? (
          ...
          {selectedProvider && selectedModelId && models && (
            <div className="mb-4 text-sm text-gray-500 dark:text-gray-400">
              <p>
                {getModelDescription(models, selectedProvider, selectedModelId)}
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <textarea
              ...
              disabled={
                isLoadingResponse || isLoadingContent || !selectedModelId
              }
            />
            <button
              type="submit"
